...
// this is because the game doesn't compile outside debug :(
#define _ALLOW_ITERATOR_DEBUG_LEVEL_MISMATCH

// some old graphics library that was apparently used in 1996
...
void drawteras(int, int, int);
void drawobj(int, int, int, int, int);
void moveteras(int, int, int*, int);
void eaten(void);
void getobj(int, int, int, int);
void getimg(int, int, int, int, void**);
void drawkeli(void);
void drawskala(void);
void getscala(void);
void pushbutton(int);
...
int radius, c, lefteras, righteras, lefterd, righterd;
float ratio;
void* p, * q, * r, * s, * t;
int a, b, xasp, yasp, leftman, rightman, topman, size, rule, get, getd, dun;
char d;
int it, itd, i, getsk, pushed, pushd, putsk, leftsk, upsk, j;
int onsk, maxj, getkl, freeg, freed, maxupsk, gate;
float tport;
...
void init() {
    rule = 1, get = -1, getd = -1, dun = 0;
    // ????
    d = 'g';
    it = 1, itd = 1, i = -1, getsk = -1, pushed = 0, pushd = 0, putsk = 0, leftsk = -1, upsk = 0, j = 0;
    onsk = 0, maxj = -1, getkl = -1, freeg = 0, freed = 0, maxupsk = -1, gate = 0;
    tport = 1.0;
    randomize();
    a = DETECT;
    if (!hasInit)
        initwindow(630, 470, "hi");
    hasInit = true;
    getaspectratio(&xasp, &yasp);
    ratio = (float)xasp / yasp;
    rectangle(0, 0, getmaxx(), getmaxy());
    line(getmaxx() / 100, getmaxy() / 1.49, getmaxx() / 1.01, getmaxy() / 1.49);
    radius = getmaxx() / 25;
    // start player
    circle(getmaxx() / 15, getmaxy() / 1.785, radius);
    line(getmaxx() / 15, getmaxy() / 1.5 - ratio * radius, getmaxx() / 15 + radius, getmaxy() / 1.5);
    line(getmaxx() / 15, getmaxy() / 1.5 - ratio * radius, getmaxx() / 15 - radius, getmaxy() / 1.5);
    topman = getmaxy() / 1.785 - ratio * radius;
    leftman = getmaxx() / 15 - radius;
    rightman = getmaxx() / 15 + radius;
    // end player
    ...
    lefteras = 14 * getmaxx() / 15 - radius;
    righteras = lefteras + 2 * radius;
    lefterd = 14 * getmaxx() / 15 - radius;
    righterd = lefterd + 2 * radius;
    // first button
    drawobj(2 * getmaxx() / 3, getmaxy() / 1.785, radius / 3, ratio * radius / 2, 0);
    ...
    line(getmaxx() / 2 - 0.75 * radius, getmaxy() / 9 + 4 * ratio * radius, getmaxx() / 2 + 0.75 * radius, getmaxy() / 9 + 4 * ratio * radius);
    buttons();
}
...
void buttons() {
    while ((c = getch()) != '1' && freed != 4) {
        switch (c) {
        case 'w':
            if (putsk == 1 && leftman == leftsk && rule == 1 && topman > getmaxy() / 9 + 2 * ratio * radius) {
                onsk = 1;
                ...
                topman -= ratio * radius;
                putimage(getmaxx() / 15 + i * radius, getmaxy() / 1.785 - ratio * radius - upsk, p, 1);
                if (topman <= getmaxy() / 9 + 2 * ratio * radius) {
                    maxj = j;
                    maxupsk = upsk;
                }
            }
                ...
                if (rightman >= 14 * getmaxx() / 15 && topman == (int)(1.59 * getmaxy() / 1.785 - 2 * ratio * radius) && getd == -1) {
                    getobj(14 * getmaxx() / 15 - radius / 3, 1.59 * getmaxy() / 1.785 - 1.25 * ratio * radius, ratio * radius / 2, 1);
                    getd = 1;
                }
            ...
            if (leftman > getmaxx() / 15 - radius) {
                if (onsk == 1) {
                    ...
                    if (freeg > 0 && freed == 0) {
                        ...
                        putimage(getmaxx() / 15 + (i + 1) * radius, getmaxy() / 1.785 - ratio * radius - maxupsk, p, 1);
                        freed = 1;
                    }
                }
                else {
                ...
                if (rightman < getmaxx() - 2 * radius && rightman > getmaxx() - 3 * radius && freed == 1 && rule == 1) {
                    int g = i;
                    putimage(getmaxx() / 15 + (g + 2) * radius, getmaxy() / 1.785 - ratio * radius - maxupsk, p, 1);
                    putimage(getmaxx() / 15 + (g + 1) * radius, getmaxy() / 1.785 - ratio * radius - maxupsk, p, 1);
                    while (maxupsk) {
                        putimage(getmaxx() / 15 + (g + 1) * radius, getmaxy() / 1.785 - ratio * radius - maxupsk, p, 1);
                        maxupsk -= 0.2 * ratio * radius;
                        putimage(getmaxx() / 15 + (g + 1) * radius, getmaxy() / 1.785 - ratio * radius - maxupsk, p, 1);
                    }++freed;
                }
            }
        ...
        case 'd':
            ...
            break;
        case 's':
            if (leftman == getmaxx() / 15 - radius && rule == 0 && pushd == 0 && dun == 0) {
                pushbutton(0);
                drawkey();
                pushd = 1;
                ...
                if (leftman == getmaxx() / 15 - radius && rule == 1 && getsk == -1 && pushed == 1) {
                    ...
                    if (onsk == 1) {
                        if (j == maxj) {
                            ...
                            else if (rightman >= 14 * getmaxx() / 15 && freeg == 0 && getkl == 1) {
                                putimage(7.05 * getmaxx() / 8 + 1, getmaxy() / 9 + 1, t, 1);
                                freeg = 1;
                                setviewport(7.05 * getmaxx() / 8 + 0.9 * radius, getmaxy() / 9 + 0.19 * ratio * radius, 7.05 * getmaxx() / 8 + 0.9 * radius + textwidth((char*)
                                    "HELP"), getmaxy() / 9 + 0.19 * ratio * radius + 0.7 * textheight((char*)
                                        "HELP"), 1);
                                clearviewport();
                        ...
                        if (get == 1) {
                            if (topman == (int)(getmaxy() / 1.785 - ratio * radius)) {
                                tport = 1.59;
                                putimage(getmaxx() / 15 + i * radius, getmaxy() / 1.785 - ratio * radius, p, 1);
                                if (freed == 2) putimage(getmaxx() / 15 + (i + 1) * radius, getmaxy() / 1.785 - ratio * radius, p, 1);
                                rule = 0;
                            }
                            if (topman == (int)(1.59 * getmaxy() / 1.785 - ratio * radius)) {
                                tport = 1.0;
                                putimage(getmaxx() / 15 + i * radius, 1.59 * getmaxy() / 1.785 - ratio * radius, p, 1);
                                if (freed == 2) putimage(getmaxx() / 15 + (i + 1) * radius, 1.59 * getmaxy() / 1.785 - ratio * radius, p, 1);
                                rule = 1;
                            }
                            putimage(getmaxx() / 15 + i * radius, tport * getmaxy() / 1.785 - ratio * radius, p, 1);
                            if (freed == 2) putimage(getmaxx() / 15 + (i + 1) * radius, tport * getmaxy() / 1.785 - ratio * radius, p, 1);
                            topman = tport * getmaxy() / 1.785 - ratio * radius;
                        }
            break;
        case 'e':
            if (!onsk && freed != 2) {
                if (rightman < getmaxx() - radius) {
                    ...
                    moveteras(14 * getmaxx() / 15, 1.59 * getmaxy() / 1.785, &itd, random(2));
                    putimage(getmaxx() / 15 + i * radius, tport * getmaxy() / 1.785 - 2 * ratio * radius, p, 1);
                    if (rightman < getmaxx() - 1.5 * radius) {
                        ++i;
                        leftman += radius;
                        rightman += radius;
                    }
        ...
        case 'x':
            if (j == 0) {
                if (putsk == 0 && rule == 1 && getsk == 1) {
                    putimage(getmaxx() / 15 + i * radius, getmaxy() / 9 + 4 * ratio * radius, r, 1);
                    putsk = 1;
                    leftsk = leftman;
                }
                else
                    if (rule == 1 && leftsk == leftman && freed % 3 == 0) {
                        putimage(getmaxx() / 15 + i * radius, getmaxy() / 9 + 4 * ratio * radius, r, 1);
                        putsk = 0;
                    }
            }
        ...
        // Click the first button (that unlocks bottom movement)
        if (rightman >= 2 * getmaxx() / 3 && get == -1) {
            getobj(2 * getmaxx() / 3, getmaxy() / 1.785, ratio * radius / 2, 0);
            get = 1;
        ...
        if (rightman >= lefteras && rule == 1 && leftman <= righteras || rightman >= lefterd && rule == 0 && leftman <= righterd) {
            break;
        }
        if (leftman <= getmaxx() / 2 + 0.75 * radius && rightman >= getmaxx() / 2 + 0.75 * radius && rule == 1 && freeg == 2) {
            break;
        }
        if (getd == 1 && dun == 0) {
            drawkeli();
        ...
        if (leftman == getmaxx() / 15 - radius && rule == 1 && gate && (freed == 2 || putsk == 0)) {
            ...
            delay(200);
            leftman += radius;
            putimage(getmaxx() / 15 - radius, getmaxy() / 1.785 - ratio * radius, p, 1);
            ++i;
            ++freed;
        }
    }
    if (freed == 4) {
        ...
        clearviewport();
        setviewport(0, 0, getmaxx(), getmaxy(), 1);
        outtextxy(2 * getmaxx() / 5, 2 * getmaxy() / 5, (char*)
            "CONGRATULATIONS!!!");
        outtextxy(2 * getmaxx() / 5, 2 * getmaxy() / 5 + textheight((char*)
            "T"), (char*)
            "Another game pal? :-) (y/n)");
        while (d != 'y' && d != 'n') {
            if ((d = getch()) == 'y') { 
                init();
                clearviewport();
...
void drawteras(int xter, int yter, int down) {
    line(xter, yter - ratio * radius, xter - radius, yter);
    moveto(xter - radius, yter);
    linerel(radius, 0);
    lineto(xter - radius, down - ratio * radius);
    linerel(radius, 0);
    linerel(0, ratio * radius);
    linerel(radius, 0);
    lineto(xter + radius, yter - ratio * radius);
    linerel(-radius, 0);
}
...
void moveteras(int xmter, int ymter, int* d, int flag) {
    ...
    if (righter < xmter + radius && flag == 0) {
        ...
        if (righteras < xmter + radius) flag = 0;
        else flag = 1;
    }
}
void drawobj(int xobj, int yobj, int xwidth, int yheight, int x) {
    rectangle(xobj, yobj, xobj + xwidth, yobj - yheight);
    if (x) {
...
void drawkeli(void) {
    ...
    putimage(7.05 * getmaxx() / 8 + 0.5 * radius, getmaxy() / 9 + 0.9 * ratio * radius, p, 1);
    for (i = 0; i < 3; ++i) {
        outtextxy(7.05 * getmaxx() / 8 + 0.9 * radius, getmaxy() / 9 + 0.19 * ratio * radius, (char*)
            "HELP");
        if (i == 2) {
            break;
        }
...
void pushbutton(int px) {
    ...
    clearviewport();
    setviewport(0, 0, getmaxx(), getmaxy(), 1);
    drawobj(getmaxx() / 100, getmaxy() / 1.1, radius / 6, ratio * radius / 2, px);
    delay(70);
}
void getscala(void) {
...
void drawgate(void) {
    ...
    outtextxy(getmaxx() / 15 - 0.6 * radius, getmaxy() / 1.785 - 1.2 * ratio * radius - textheight((char*)
        "EXIT"), (char*)
        "EXIT");
}